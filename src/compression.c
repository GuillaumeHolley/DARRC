...
extern inline Read_count* create_Read_count();
extern inline Pos_length_occ* create_pos_length_occ();
extern inline void set_Pos_length_occ(Pos_length_occ* pos_length_occ, int pos, int length, int occ_count,
                               int rev, int nb_mismatches, int64_t* id_occ, Mismatch* list_mismatches);
extern inline Pos_length_occ* create_set_Pos_length_occ(int pos, int length, int occ_count, int rev, int nb_mismatches,
                                                 int64_t* id_occ, Mismatch* list_mismatches);
extern inline Pos_length_occ* create_set_Pos_length_occ_from_Read_count(int pos, int length, int rev, const Read_count* read_count);
extern inline int custom_atoi(const char *old_str, char** new_str, char end);
extern inline int custom_strstr_dna_triplet(const char *str, int len, bool upper_char, bool check_last_triplet_only);
...
void decompress_FASTx_file(char* output_prefix, BFT_Root* graph_no_iupac, BFT_Root* graph_iupac, bool pair_ended, int size_kmer, int size_seed){
    ...
    decompress2(output_prefix_buffer, graph_no_iupac, graph_iupac, pair_ended, size_kmer, size_seed);
...
int64_t binning_reads(char* filename_reads_mate_1, char* filename_reads_mate_2, bool pair_ended, char* prefix_bin_name, int size_minimizer){
    ...
    int length_read;
    int fp_read;
    int pos_char_non_acgt;
    int pos_curr_minimzer;

    int minimizer_shift = 0;
    int length_buffer_read_rev = SIZE_BUFFER;
    int nb_min_per_bin = CEIL((int)pow(4, size_minimizer), FOPEN_MAX_HARD - 64);
    ...
    bool prev_min_is_safe, prev_min_is_safe2;
    bool min_is_rev;
    ...
    int* files = malloc((FOPEN_MAX_HARD - 64) * sizeof(int));
    ASSERT_NULL_PTR(files, "binning_reads()\n")
    ...
    for (i = 0; i < FOPEN_MAX_HARD - 64; i++){

        sprintf(buffer_int, "_super%d", i);
        strcpy(&bin_name[length_prefix_bin_name], buffer_int);

        files[i] = open(bin_name, O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IWUSR);
        if (files[i] == -1) ERROR("binning_reads(): Could not open/create the file corresponding to a bin")
    }
    ...
    while (kseq_read(seq_read) > -1){
        ...
        min_is_rev = false;
        prev_min_is_safe = false;
        prev_min_is_safe2 = false;

        for (i = 0; i <= length_read - size_minimizer; i++){

            pos_char_non_acgt = is_substring_nonACGT(&(seq_read->seq.s[i]), size_minimizer, prev_min_is_safe && prev_min_is_safe2);
            prev_min_is_safe = pos_char_non_acgt == -1;

            if (prev_min_is_safe){

                pos_char_non_acgt = custom_strstr_dna_triplet(&(seq_read->seq.s[i]), size_minimizer, true, prev_min_is_safe2);
                prev_min_is_safe2 = pos_char_non_acgt == -1;

                if (prev_min_is_safe2){

                    pos_curr_minimzer = i;
                    curr_minimizer = &(seq_read->seq.s[i]);
                    break;
                }
                else i += pos_char_non_acgt;
            }
            else i += pos_char_non_acgt;
        }
        ...
        if (pos_curr_minimzer != -1){
            ...
            for (i=0; i <= length_read - size_minimizer; i++){
                ...
                pos_char_non_acgt = is_substring_nonACGT(&buffer_read_rev[i], size_minimizer, prev_min_is_safe && prev_min_is_safe2);
                prev_min_is_safe = pos_char_non_acgt == -1;

                if (prev_min_is_safe){

                    pos_char_non_acgt = custom_strstr_dna_triplet(&buffer_read_rev[i], size_minimizer, true, prev_min_is_safe2);
                    prev_min_is_safe2 = pos_char_non_acgt == -1;
        ...
        minimizer /= nb_min_per_bin;

        sprintf(buffer_int, "%d ", pos_curr_minimzer);
        write(files[minimizer], buffer_int, strlen(buffer_int));

        write(files[minimizer], buffer, length_read);

        if (min_is_rev) write(files[minimizer], info_read_rev, 2);
        else write(files[minimizer], info_read_non_rev, 2);
    ...
    for (i = 0; i < FOPEN_MAX_HARD - 64; i++) close(files[i]);
    free(files);
...
char** subdivide_bin(char* prefix_bin_name, uint64_t id_bin, int size_minimizer, bool delete_id_bin_file){
    ...
    uint64_t minimizer, minimizer_files;

    char buffer_int[64] = {0};

    char* delimiter;

    char** filenames = malloc(nb_min_per_bin * sizeof(char*));
    ASSERT_NULL_PTR(filenames, "subdivide_bin()\n")

    char* bin_name = malloc((length_prefix_bin_name + size_minimizer + 100) * sizeof(char));
    ASSERT_NULL_PTR(bin_name, "subdivide_bin()\n")

    char* buffer_read = malloc(size_buffer_read * sizeof(char));
    ASSERT_NULL_PTR(buffer_read, "subdivide_bin()\n")

    int* files = malloc(nb_min_per_bin * sizeof(int));
    ASSERT_NULL_PTR(files, "subdivide_bin()\n")

    uint8_t* id_minimizer = malloc(size_minimizer_bytes * sizeof(uint8_t));
    ASSERT_NULL_PTR(id_minimizer, "subdivide_bin()\n")

    FILE* file_bin = NULL;

    strcpy(bin_name, prefix_bin_name);

    sprintf(buffer_int, "_super%" PRIu64, id_bin);
    strcpy(&bin_name[length_prefix_bin_name], buffer_int);

    file_bin = fopen(bin_name, "r");
    ASSERT_NULL_PTR(file_bin, "subdivide_bin()\n")

    if (size_minimizer % 4) minimizer_shift = SIZE_BITS_UINT_8T - ((size_minimizer % 4) * 2);

    for (i = 0; i < nb_min_per_bin; i++){
        files[i] = -1;
        filenames[i] = NULL;
    }
    ...
    while (getline(&buffer_read, &size_buffer_read, file_bin) != -1){
        ...
        if (pos_minimizer >= 0){
            ...
            memset(id_minimizer, 0, size_minimizer_bytes * sizeof(uint8_t));
            parseKmerCount(&delimiter[pos_minimizer], size_minimizer, id_minimizer, 0);
        ...
        minimizer_files = minimizer % nb_min_per_bin;

        if (files[minimizer_files] == -1){

            sprintf(buffer_int, "_%" PRIu64, minimizer);
            strcpy(&bin_name[length_prefix_bin_name], buffer_int);

            files[minimizer_files] = open(bin_name, O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IWUSR);
            if (files[minimizer_files] == -1) ERROR("subdivide_bin(): Could not open/create the file corresponding to a bin\n")

            filenames[minimizer_files] = bin_name;

            bin_name = malloc((length_prefix_bin_name + size_minimizer + 100) * sizeof(char));
            ASSERT_NULL_PTR(bin_name, "subdivide_bin()\n")

            strcpy(bin_name, prefix_bin_name);
        }

        write(files[minimizer_files], buffer_read, strlen(buffer_read));
    }
    ...
    if (delete_id_bin_file){

        sprintf(buffer_int, "_super%" PRIu64, id_bin);
        strcpy(&bin_name[length_prefix_bin_name], buffer_int);

        if (remove(bin_name)) ERROR("subdivide_bin(): could not delete file corresponding to id_bin\n")
    }
...
void reassemble_bins_to_super_bins(char* prefix_bin_name, uint64_t id_super_bin, char** filenames_bin, int size_minimizer){
    ...
    char* bin_name = malloc((length_prefix_bin_name + size_minimizer + 100) * sizeof(char));
    ASSERT_NULL_PTR(bin_name, "reassemble_bins_to_super_bins()\n")

    long int* offsets = malloc((nb_min_per_bin + 1) * sizeof(long int));
    ASSERT_NULL_PTR(offsets, "reassemble_bins_to_super_bins()\n")

    uint8_t* buffer = malloc(SIZE_BUFFER * sizeof(uint8_t));
    ASSERT_NULL_PTR(buffer, "reassemble_bins_to_super_bins()\n");

    FILE* file_super_bin = NULL;
    FILE* file_append = NULL;

    strcpy(bin_name, prefix_bin_name);

    sprintf(buffer_int, "_super%" PRIu64, id_super_bin);
    strcpy(&bin_name[length_prefix_bin_name], buffer_int);

    file_super_bin = fopen(bin_name, "w");
    ASSERT_NULL_PTR(file_super_bin, "reassemble_bins_to_super_bins()\n")

    fseek(file_super_bin, (nb_min_per_bin + 1) * sizeof(long int), SEEK_SET);

    for (i = 0; i < nb_min_per_bin; i++){

        if (filenames_bin[i] != NULL){

            offsets[i] = ftell(file_super_bin);

            file_append = fopen(filenames_bin[i], "r");
            ASSERT_NULL_PTR(file_append, "reassemble_bins_to_super_bins()\n");

            while (feof(file_append) == 0){
                size_buff = fread(buffer, sizeof(uint8_t), SIZE_BUFFER, file_append);
                fwrite(buffer, sizeof(uint8_t), size_buff, file_super_bin);
            }

            fclose(file_append);

            if (remove(filenames_bin[i])) ERROR("reassemble_bins_to_super_bins(): Could not remove bin file.\n");
        }
        else if (i > 0) offsets[i] = offsets[i-1];
        else offsets[0] = ftell(file_super_bin);
    }

    offsets[i] = ftell(file_super_bin);

    rewind(file_super_bin);
    fwrite(offsets, sizeof(long int), nb_min_per_bin + 1, file_super_bin);
    fclose(file_super_bin);
...
void create_super_reads(char* prefix_bin_name, bool pair_ended, int size_minimizer){
    ...
    for (it_bins = 0; it_bins < FOPEN_MAX_HARD - 64; it_bins++){

        filenames = subdivide_bin(prefix_bin_name, it_bins, size_minimizer, true);

        for (it_buffer = 0; it_buffer < nb_min_per_bin; it_buffer++){

            if (filenames[it_buffer] != NULL){

                file_bin = fopen(filenames[it_buffer], "r");
                ...
                while (reads_value != NULL){
                    ...
                    max_list_count = MAX(max_list_count, list_count);
                ...
                //Create super reads
                file_bin = fopen(filenames[it_buffer], "w");
                ASSERT_NULL_PTR(file_bin, "binning_reads()\n")
        ...
        reassemble_bins_to_super_bins(prefix_bin_name, it_bins, filenames, size_minimizer);

        for (it_buffer = 0; it_buffer < nb_min_per_bin; it_buffer++){
            if (filenames[it_buffer] != NULL) free(filenames[it_buffer]);
        }
...
uint64_t create_spanning_super_reads(char* prefix_bin_name, bool pair_ended, int64_t nb_reads_per_file, int size_minimizer){
    ...
    long int pos_curr_line_tmp_tmp;

    long int pos_start_line_tmp;
    long int pos_curr_line_tmp;

    long int pos_end_bin, pos_end_bin_tmp;

    bool continue_loop;
    bool first_pos_length_occ;
    bool is_rev;
    bool prev_min_is_safe;
    bool prev_min_is_safe2;
    ...
    char* err_read = "create_spanning_super_reads(): cannot read in bin\n";
    char* err_write = "create_spanning_super_reads(): cannot write in bin\n";
    ...
    files = malloc((FOPEN_MAX_HARD - 64) * sizeof(FILE*));
    ASSERT_NULL_PTR(files, "create_spanning_super_reads()\n")
    ...
    for (it_buffer = 0; it_buffer < FOPEN_MAX_HARD - 64; it_buffer++){

        sprintf(buffer_int, "_super%" PRIu64, it_buffer);
        strcpy(&bin_name[length_prefix_bin_name], buffer_int);

        files[it_buffer] = fopen(bin_name, "r+");
        ASSERT_NULL_PTR(files[it_buffer], "create_spanning_super_reads()\n")
    }
    ...
    for (it_buffer = 0; it_buffer < pow(4, size_minimizer); it_buffer++){
        ...
        /*sprintf(buffer_int, "_super%" PRIu64, it_buffer / nb_min_per_bin);
        strcpy(&bin_name[length_prefix_bin_name], buffer_int);
        files[it_buffer / nb_min_per_bin] = fopen(bin_name, "r+");
        ASSERT_NULL_PTR(files[it_buffer / nb_min_per_bin], "create_spanning_super_reads()\n");*/

        file_bin = files[it_buffer / nb_min_per_bin];

        fseek(file_bin, (it_buffer % nb_min_per_bin) * sizeof(long int), SEEK_SET);
        if (fread(&pos_start_line, sizeof(long int), 1, file_bin) != 1) ERROR(err_read);
        if (fread(&pos_end_bin, sizeof(long int), 1, file_bin) != 1) ERROR(err_read);
        fseek(file_bin, pos_start_line * sizeof(char), SEEK_SET);

        while ((pos_start_line < pos_end_bin) && (getline(&buffer_read, &size_buffer_read, file_bin) != -1)){
            ...
            if (buffer_read[0] != read_used){
                ...
                for (i = custom_atoi(buffer_read, &delimiter_read_tmp, eol) + 1; i <= length_read - size_minimizer; i++){
                    ...
                    pos_char_non_acgt = is_substring_nonACGT(&read[i], size_minimizer, prev_min_is_safe && prev_min_is_safe2);
                    prev_min_is_safe = pos_char_non_acgt == -1;

                    if (prev_min_is_safe){

                        pos_char_non_acgt = custom_strstr_dna_triplet(&read[i], size_minimizer, true, prev_min_is_safe2);
                        prev_min_is_safe2 = pos_char_non_acgt == -1;
                ...
                if (minimizer != opened_file_minimizer){
                    ...
                    pos_curr_line_tmp_tmp = ftell(file_bin);

                    continue_loop = true;

                    while (continue_loop){

                        /*sprintf(buffer_int, "_super%" PRIu64, minimizer / nb_min_per_bin);
                        strcpy(&bin_name[length_prefix_bin_name], buffer_int);
                        files[minimizer / nb_min_per_bin] = fopen(bin_name, "r+");
                        ASSERT_NULL_PTR(files[minimizer / nb_min_per_bin], "create_spanning_super_reads()\n");*/

                        file_new_bin = files[minimizer / nb_min_per_bin];

                        fseek(file_new_bin, (minimizer % nb_min_per_bin) * sizeof(long int), SEEK_SET);
                        if (fread(&pos_start_line_tmp, sizeof(long int), 1, file_new_bin) != 1) ERROR(err_read);
                        if (fread(&pos_end_bin_tmp, sizeof(long int), 1, file_new_bin) != 1) ERROR(err_read);
                        fseek(file_new_bin, pos_start_line_tmp * sizeof(char), SEEK_SET);

                        continue_loop = false;

                        while ((pos_start_line_tmp < pos_end_bin_tmp) && (getline(&buffer_read_tmp, &size_buffer_read_tmp, file_new_bin) != -1)){
                                    ...
                                    if (match == 0){
                                        ...
                                        if (match == 0){
                                            ...
                                            if (length_spanning == 1){
                                                ...
                                                if (fwrite(&read_used, sizeof(char), 1, file_bin) != 1) ERROR(err_write);
                                            ...
                                            if (fwrite(&read_used, sizeof(char), 1, file_new_bin) != 1) ERROR(err_write);
                                            ...
                                            for (i = pos_minimizer_tmp; i <= length_read - size_minimizer; i++){
                                                ...
                                                pos_char_non_acgt = is_substring_nonACGT(&buffer_read[i], size_minimizer, prev_min_is_safe && prev_min_is_safe2);
                                                prev_min_is_safe = pos_char_non_acgt == -1;

                                                if (prev_min_is_safe){

                                                    pos_char_non_acgt = custom_strstr_dna_triplet(&buffer_read[i], size_minimizer, true, prev_min_is_safe2);
                                                    prev_min_is_safe2 = pos_char_non_acgt == -1;
                    ...
                    fseek(file_bin, pos_curr_line_tmp_tmp, SEEK_SET);
                }
            }
    ...
    for (it_buffer = 0; it_buffer < pow(4, size_minimizer); it_buffer++){
        ...
        fseek(file_bin, (it_buffer % nb_min_per_bin) * sizeof(long int), SEEK_SET);
        fread(&pos_start_line, sizeof(long int), 1, file_bin);
        fread(&pos_end_bin, sizeof(long int), 1, file_bin);

        fseek(file_bin, pos_start_line * sizeof(char), SEEK_SET);

        while ((ftell(file_bin) < pos_end_bin) && (getline(&buffer_read, &size_buffer_read, file_bin) != -1)){
    ...
    fseek(file_span_super_reads, 0 - ((long int) sizeof(char)), SEEK_CUR);
...
uint32_t compressKmers_from_KmerFiles_bis3(char* output_prefix, int size_seed, int size_kmer, uint32_t prev_nb_parts){
    ...
    while ((j = fread(kmers_iupac, nb_bytes_kmer_iupac * sizeof(uint8_t), nb_kmer_in_buf, file_kmers))){
        ...
        for (i = 0; i < j; i++, kmers_read++){
            ...
            pref_iupac = is_substring_nonACGT(prefixes_iupac, size_seed, false) != -1;
            ...
            suf_iupac = is_substring_nonACGT(suffixes_iupac, size_seed, false) != -1;
...
void serialize_subpaths_recycling(char* filename_subpaths_recycling, Pvoid_t* recycl_subpaths, int size_seed){
    ...
    while (PValue != NULL){
        ...
        if (is_substring_nonACGT((char*)seed_ascii, size_seed, false) == -1){
...
void insert_kmers_partitions2(char* output_prefix, int size_seed, int size_kmer, bool recycle_paths,
    ...
    if ((root_no_IUPAC == NULL) && (root_IUPAC == NULL)){
        ...
        root_no_IUPAC->ann_inf = create_annotation_inform(-1, true);
        root_IUPAC->ann_inf = create_annotation_inform(-1, true);
    ...
    while ((j = fread(kmers_iupac, nb_bytes_kmer_iupac * sizeof(uint8_t), nb_kmer_in_buf, file_kmers))){
        ...
        for (i = 0; i < j; i++){
            ...
            if (is_substring_nonACGT(line, size_kmer, false) != -1){
    ...
    compress_annotations_disk(root_no_IUPAC, output_prefix_buffer);
    write_BFT_Root(root_no_IUPAC, output_prefix_buffer, false);
    freeBFT_Root(root_no_IUPAC);

    strcpy(&(output_prefix_buffer[len_output_prefix]), "_graph_iupac");
    compress_annotations_disk(root_IUPAC, output_prefix_buffer);
    write_BFT_Root(root_IUPAC, output_prefix_buffer, false);
...
void create_subgraph_decomp(char* output_prefix, FILE* file_partitions, Pvoid_t* prefix_part_recycl,
    ...
    new_graph_no_iupac->ann_inf = create_annotation_inform(-1, true);
    ...
    new_graph_iupac->ann_inf = create_annotation_inform(-1, true);
    ...
    strcpy(&(output_prefix_buffer[len_output_prefix]), "_graph_no_iupac");
    compress_annotations_disk(new_graph_no_iupac, output_prefix_buffer);
    write_BFT_Root(new_graph_no_iupac, output_prefix_buffer, false);
    freeBFT_Root(new_graph_no_iupac);

    strcpy(&(output_prefix_buffer[len_output_prefix]), "_graph_iupac");
    compress_annotations_disk(new_graph_iupac, output_prefix_buffer);
    write_BFT_Root(new_graph_iupac, output_prefix_buffer, false);
    freeBFT_Root(new_graph_iupac);

    *graph_iupac = read_BFT_Root(output_prefix_buffer);
    remove(output_prefix_buffer);

    strcpy(&(output_prefix_buffer[len_output_prefix]), "_graph_no_iupac");
    *graph_no_iupac = read_BFT_Root(output_prefix_buffer);
    remove(output_prefix_buffer);
...
void cdbg_traverse_overlap_insert(BFT* bft_no_iupac, BFT* bft_iupac, BFT* insert_bft_no_iupac, BFT* insert_bft_iupac,
    ...
    while (kmer_has_partition){
        ...
        if (is_substring_nonACGT(overlap, length_overlap, false) != -1){
...
uint8_t find_kmer(BFT* graph_iupac, BFT* graph_no_iupac,
    ...
    prefix_is_IUPAC = is_substring_nonACGT(*seed, length_seed, false) != -1;
    ...
    if (ret == 3){
        ...
        if (kmer_found){ // Such a k-mer is found
            ...
            if (is_substring_nonACGT(&((*seed)[graph_no_iupac->k - length_seed]), length_seed, false) != -1){
...
void insert_overlap(char* overlap, int length_overlap, uint8_t* partition_no_iupac, Pvoid_t* partition_iupac, Pvoid_t* pref_suf_pos){
    ...
    if (is_substring_nonACGT(overlap, length_overlap, false) != -1) JSLI(PValue_partition_iupac, *partition_iupac, (uint8_t*) overlap)
    else{
...
void write_meta_info_file(const char* output_prefix, bool paired, const char* version_tool){
    ...
    ASSERT_NULL_PTR(output_prefix, "write_meta_info_file()\n");
    ASSERT_NULL_PTR(version_tool, "write_meta_info_file()\n");

    FILE* file;

    uint8_t pair_info = paired ? 1 : 0;

    char* output_prefix_buffer = malloc((strlen(output_prefix) + 50) * sizeof(char));
    ASSERT_NULL_PTR(output_prefix_buffer, "write_meta_info_file()\n");

    strcpy(output_prefix_buffer, output_prefix);
    strcpy(&output_prefix_buffer[strlen(output_prefix_buffer)], "_info");

    file = fopen(output_prefix_buffer, "wb");
    ASSERT_NULL_PTR(file, "write_meta_info_file()\n");

    if (fwrite(version_tool, sizeof(char), strlen(version_tool) + 1, file) != (strlen(version_tool) + 1))
        ERROR("write_meta_info_file(): could not write meta data _info file.\n");

    if (fwrite(&pair_info, sizeof(uint8_t), 1, file) != 1)
        ERROR("write_meta_info_file(): could not write meta data _info file.\n");
...
bool read_meta_info_file(char* input_prefix){
    ...
    ASSERT_NULL_PTR(input_prefix, "read_meta_info_file()\n");

    FILE* file;

    uint8_t pair_info;

    char c = 'x'; //Anything except '\0'

    char* input_prefix_buffer = malloc((strlen(input_prefix) + 50) * sizeof(char));
    ASSERT_NULL_PTR(input_prefix_buffer, "read_meta_info_file()\n");

    strcpy(input_prefix_buffer, input_prefix);
    strcpy(&input_prefix_buffer[strlen(input_prefix_buffer)], "_info");

    file = fopen(input_prefix_buffer, "rb");
    ASSERT_NULL_PTR(file, "read_meta_info_file()\n");

    while (c != '\0') c = getc(file);

    if (fread(&pair_info, sizeof(uint8_t), 1, file) != 1)
        ERROR("read_meta_info_file(): could not read meta data _info file.\n");

    fclose(file);

    free(input_prefix_buffer);
...
void append_file(char* filename_base, char* filename_to_append){
    ...
    ASSERT_NULL_PTR(filename_base, "append_file()\n");
    ASSERT_NULL_PTR(filename_to_append, "append_file()\n");

    size_t size_buff;

    FILE* file_base = fopen(filename_base, "ab");
    ASSERT_NULL_PTR(file_base, "append_file()\n");

    FILE* file_append = fopen(filename_to_append, "rb");
    ASSERT_NULL_PTR(file_append, "append_file()\n");

    uint8_t* buffer = malloc(SIZE_BUFFER * sizeof(uint8_t));
    ASSERT_NULL_PTR(buffer, "append_file()\n");

    while (feof(file_append) == 0){
        size_buff = fread(buffer, sizeof(uint8_t), SIZE_BUFFER, file_append);
        fwrite(buffer, sizeof(uint8_t), size_buff, file_base);
    }
...
void write_graph_info_file(char* graph, long int offset_graph_iupac, long int offset_info_file,
                           uint32_t nb_parts, int length_overlap, int pos_path_recycling,
                           int threshold_path_recycling, const char* version_tool){

    ASSERT_NULL_PTR(graph, "write_graph_info_file()\n");
    ASSERT_NULL_PTR(version_tool, "write_graph_info_file()\n");

    FILE* file = fopen(graph, "ab");
    ASSERT_NULL_PTR(file, "write_graph_info_file()\n");

    if (fwrite(version_tool, sizeof(char), strlen(version_tool) + 1, file) != (strlen(version_tool) + 1))
        ERROR("write_graph_info_file(): could not write info to graph file.\n");

    if (fwrite(&nb_parts, sizeof(uint32_t), 1, file) != 1)
        ERROR("write_graph_info_file(): could not write info to graph file.\n");

    if (fwrite(&length_overlap, sizeof(int), 1, file) != 1)
        ERROR("write_graph_info_file(): could not write info to graph file.\n");

    if (fwrite(&offset_graph_iupac, sizeof(long int), 1, file) != 1)
        ERROR("write_graph_info_file(): could not write info to graph file.\n");

    if (fwrite(&offset_info_file, sizeof(long int), 1, file) != 1)
        ERROR("write_graph_info_file(): could not write info to graph file.\n");

    if (fwrite(&pos_path_recycling, sizeof(int), 1, file) != 1)
        ERROR("write_graph_info_file(): could not write info to graph file.\n");

    if (fwrite(&threshold_path_recycling, sizeof(int), 1, file) != 1)
        ERROR("write_graph_info_file(): could not write info to graph file.\n");
